# -*- coding: utf-8 -*-
# Copyright 2017 Google Inc and Microsoft Corp.
#
...
EMPTY = '<empty>'  # MZ: added to substitute empty reference/hypothesis
...
def rouge_n(evaluated_sentences, reference_sentences, n=2):
    ...
    """
    # MZ: added hacky way to handle empty sentences
    if len(evaluated_sentences) <= 0:
        evaluated_sentences = EMPTY
        #raise ValueError("Hypothesis is empty.")
    if len(reference_sentences) <= 0:
        reference_sentences = EMPTY
...
def rouge_l_summary_level(evaluated_sentences, reference_sentences):
    ...
    Raises:
      ValueError: raises exception if a param has len <= 0
    """
    # MZ: hacky way to handle empty sentences
    if len(evaluated_sentences) <= 0:
        evaluated_sentences = EMPTY
    if len(reference_sentences) <= 0:
        reference_sentences = EMPTY
